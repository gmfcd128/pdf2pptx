...
3. **OCR the rest via PaddleOCR** (`ocr.OcrEngine.extract`) — one local `PaddleOCR.predict()` call per page (PP-OCRv6, `engine="transformers"`, see below), pinned to PP-OCRv6's own documented default detection/recognition settings (`text_det_limit_type/side_len`, `text_det_thresh`, `text_det_box_thresh`, `text_det_unclip_ratio`) plus `text_rec_score_thresh=0`, so the model itself does no confidence-based filtering — returning per-line boxes, text, and a real confidence score for each. `extract` returns two things: `results` (boxes that passed our own `PipelineConfig.ocr_min_conf`, default 0.8, and symbol-garbage check `text_utils.looks_like_garbage` — these are what can become editable text) and `mask_boxes` (every detection, unfiltered — see step 5). PaddleOCR doesn't report font size or color, so both are derived: `ocr.sample_text_color` averages the darkest ~15% of pixels in the box (recovers colored headings, e.g. dark green, instead of flattening everything to black), and font size is estimated from box height.
4. **Reconcile native vs. OCR** (`reconcile.reconcile_native_and_ocr`) — a native line and an OCR box can overlap either because they're two detections of the same line (comparable size — keep the pixel-accurate native line, drop the OCR duplicate) or because the OCR box is a much larger block that happens to contain the native line as one small piece of it (keep the larger OCR block instead, since it has strictly more information; dropping it — the naive approach — silently throws away the rest of that text). Told apart by relative box size (`PipelineConfig.size_ratio_thresh`, `containment_thresh`). This reconciled native+OCR set (`all_texts`) is what becomes editable PPTX text boxes — it plays no part in the erasure mask (see step 5).
5. **Erase text from the background** (`inpainting.Inpainter.clean`) — the mask is built solely from the OCR model's own detections (`ocr_mask_boxes` from step 3, every box it returned, regardless of confidence/garbage filtering — text-shaped ink still needs erasing even when it's too garbled to trust as a transcription), not from the reconciled native+OCR set: native vector text still gets erased fine since OCR runs over the full rendered page image and detects it too, just via OCR's own (looser) box rather than the pixel-precise native one. Each OCR box is used as-is, unpadded — its detection already carries its own margin (`text_det_unclip_ratio`), so adding more would just double up on that (a fixed watermark region passed via `extra_boxes`, not being text-detector output, still gets a flat `PipelineConfig.mask_min_pad`). Then inpainted with LaMa (GPU neural inpainting via `simple-lama-inpainting`), downscaled to `PipelineConfig.lama_max_dim` (default 2200) first if needed to bound VRAM use, then force-resized back to the exact source `(W, H)` — LaMa pads its input to a multiple of 8 internally and doesn't always crop the output back down, so without this the background image and the text-box coordinate system silently drift apart by a few pixels.
