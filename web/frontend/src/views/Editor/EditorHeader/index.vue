      ...
      <Dropdown :trigger="['click']">
        ...
        <template #overlay>
          <Menu>
            ...
            <MenuItem @click="reuploadPdf()">重新上傳新檔案</MenuItem>
          </Menu>
        </template>
      </Dropdown>
      <Dropdown :trigger="['click']">
...
export default defineComponent({
  ...
  setup() {
    ...
    // Discards the in-memory editing session (slides, undo history, and the
    // job/page metadata backing the "PDF 背景修補" panel) and returns to the
    // upload screen -- there is no server-side project to save back to, this
    // editing session was always meant to be temporary.
    const reuploadPdf = () => {
      usePdf2pptxStore().reset()
    }
