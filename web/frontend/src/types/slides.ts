...
export interface SlideBackground {
  ...
  color?: string;
  image?: string;
  // 'stretch' is a pdf2pptx addition: non-uniform 100% x 100% stretch, not
  // available via 'cover'/'contain'/'repeat' -- see useSlideBackgroundStyle.
  // Used for converted PDF pages, whose background is placed full-bleed
  // (ignoring its own aspect ratio) the same way pdf2pptx's own PPTX export
  // does, so canvas-click-to-source-pixel math stays a simple linear scale.
  imageSize?: 'cover' | 'contain' | 'repeat' | 'stretch';
